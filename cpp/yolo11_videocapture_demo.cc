...
//储存单次识别到的白鹭坐标点
float egret_points[OBJ_NUMB_MAX_SIZE][2];
...
int main(int argc, char **argv)
{
    ...
    while(true) {
        ...
        //清空白鹭坐标点
        memset(egret_points, 0, sizeof(egret_points));
        ...
        for (int i = 0; i < od_results.count; i++)
        {
            ...
            //存储白鹭坐标点
            egret_points[i][0] = (det_result->box.left + det_result->box.right) / 2.0;
            egret_points[i][1] = (det_result->box.top + det_result->box.bottom) / 2.0;
        ...
        // for(int i = 0; i<=10; i++)
        // {
        //     printf("\negret_points:\n%f, %f ", egret_points[i][0], egret_points[i][1]);
        // }
        // 显示结果
