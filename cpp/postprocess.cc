...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "/home/cat/python_docs/lubancat_ai_manual_code/example/yolo11-myfish/model/myfish_list.txt"
...
// ==================== 新增：单输出处理函数 ====================
static int process_single_output_fp32(float *output_data, 
                                     ...
                                     float threshold)
{
    ...
    // YOLOv8/v11 单输出格式: [1, 5, 8400]
    // 5 = 4(bbox坐标) + 1(置信度)
    // 8400 = 80x80 + 40x40 + 20x20 = 6400 + 1600 + 400
    
    for (int i = 0; i < 8400; i++)
    {
        // 获取置信度 (第5个通道)
        float confidence = output_data[4 * 8400 + i];
        ...
        if (confidence > threshold)
        {
            ...
            classId.push_back(0); // 单类别，class_id=0
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    // ==================== 新增：单输出分支判断 ====================
    // 检查输出数量，如果是单输出则使用新的处理逻辑
    if (app_ctx->io_num.n_output == 1) 
    {
        printf("Using single output processing for YOLOv8/v11\n");
        
        // 获取输出属性
        rknn_tensor_attr output_attr = app_ctx->output_attrs[0];
        printf("Single output shape: [%d, %d, %d, %d]\n", 
        ...
        {
            // FP32 单输出处理
            float* output_data = (float*)_outputs[0].buf;
            validCount = process_single_output_fp32(output_data, model_in_w, model_in_h,
                                                   filterBoxes, objProbs, classId, conf_threshold);
        }
        else 
        {
            // 量化单输出处理 (如果需要，可以在这里添加)
            printf("Quantized single output not implemented yet\n");
            return -1;
        }
    }
    // ==================== 结束新增 ====================
    ...
    {
        printf("Using multi-output processing for YOLOv11\n");
        // default 3 branch
#ifdef RKNPU1
